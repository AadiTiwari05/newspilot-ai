...
class News_Authenticator():
    def __init__(self , groq_api_key, merge_prompt):
    ...
    def merge_text(self, summaries):
            ...
            Returns None if no summaries provided (instead of raising ValueError).
            Retries up to 3 times on 429 rate limit errors.
            """
            import time
            if not summaries:
                print("[merge_text] No summaries to merge — skipping this article.")
                return None
            ...
            for attempt in range(3):
                try:
                    merged_response = self.client.chat.completions.create(
                        model="llama-3.3-70b-versatile",
                        messages=[{"role": "user", "content": prompt}]
                    )
                    return merged_response.choices[0].message.content if merged_response.choices[0].message.content else "Merging failed"
                    ...
                    if '401' in err or 'invalid_api_key' in err or 'Invalid API Key' in err:
                        ...
                        wait = 35 * (attempt + 1)
                        print(f"Rate limit hit in merge_text, retrying in {wait}s... (attempt {attempt+1}/3)")
                        time.sleep(wait)
                    else:
                        print(f"Error in merge_text: {err}")
                        return None
            print("Max retries reached for merge_text.")
...
class ImgBBUploader:
    def __init__(self, api_key=None):
        if not api_key:
            raise ValueError("ImgBB API key is required. Set IMGBB_API_KEY in your .env file.")
